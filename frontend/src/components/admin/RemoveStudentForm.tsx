...
const RemoveStudentForm = ({ fetchCourseData }) => {
  ...
  const handleRemoveStudent = async (e) => {
    try {
      const assignRes = await fetch("http://127.0.0.1:5000/api/course/removeStudent", {
        method: "POST",
        headers: {
  ...
  return (
    <div >
      <button className="btn btn-quat" onClick={() => setShow(true)}>
        <h6 style={{ margin: "5px"}}>Remove a Student from a Course</h6> 
      </button>
            ...
            <Row>
              <Col>
                <Form.Control required type="tempUsername" 
                  placeholder="Please enter the student's username (ex. jvybihal)." 
                  value={tempUsername} 
